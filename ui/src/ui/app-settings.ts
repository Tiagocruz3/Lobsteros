...
import type { LobsterApp } from "./app.ts";
...
export async function refreshActiveTab(host: SettingsHost) {
  ...
  if (host.tab === "instances") {
    await loadPresence(host as unknown as LobsterApp);
  }
  if (host.tab === "sessions") {
    await loadSessions(host as unknown as LobsterApp);
  }
  if (host.tab === "cron") {
    await loadCron(host);
  }
  if (host.tab === "skills") {
    await loadSkills(host as unknown as LobsterApp);
  }
  if (host.tab === "agents") {
    await loadAgents(host as unknown as LobsterApp);
    await loadToolsCatalog(host as unknown as LobsterApp);
    await loadConfig(host as unknown as LobsterApp);
    const agentIds = host.agentsList?.agents?.map((entry) => entry.id) ?? [];
    if (agentIds.length > 0) {
      void loadAgentIdentities(host as unknown as LobsterApp, agentIds);
    }
    const agentId =
      host.agentsSelectedId ?? host.agentsList?.defaultId ?? host.agentsList?.agents?.[0]?.id;
    if (agentId) {
      void loadAgentIdentity(host as unknown as LobsterApp, agentId);
      if (host.agentsPanel === "skills") {
        void loadAgentSkills(host as unknown as LobsterApp, agentId);
      }
      if (host.agentsPanel === "channels") {
        void loadChannels(host as unknown as LobsterApp, false);
      }
  ...
  if (host.tab === "nodes") {
    await loadNodes(host as unknown as LobsterApp);
    await loadDevices(host as unknown as LobsterApp);
    await loadConfig(host as unknown as LobsterApp);
    await loadExecApprovals(host as unknown as LobsterApp);
  }
  if (host.tab === "chat") {
  ...
  if (host.tab === "config") {
    await loadConfigSchema(host as unknown as LobsterApp);
    await loadConfig(host as unknown as LobsterApp);
  }
  if (host.tab === "debug") {
    await loadDebug(host as unknown as LobsterApp);
    host.eventLog = host.eventLogBuffer;
  }
  if (host.tab === "logs") {
    host.logsAtBottom = true;
    await loadLogs(host as unknown as LobsterApp, { reset: true });
...
export async function loadOverview(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as LobsterApp, false),
    loadPresence(host as unknown as LobsterApp),
    loadSessions(host as unknown as LobsterApp),
    loadCronStatus(host as unknown as LobsterApp),
    loadDebug(host as unknown as LobsterApp),
  ]);
}

export async function loadChannelsTab(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as LobsterApp, true),
    loadConfigSchema(host as unknown as LobsterApp),
    loadConfig(host as unknown as LobsterApp),
  ]);
}

export async function loadCron(host: SettingsHost) {
  const cronHost = host as unknown as LobsterApp;
  await Promise.all([
    loadChannels(host as unknown as LobsterApp, false),
