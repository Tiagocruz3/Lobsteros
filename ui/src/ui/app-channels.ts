...
export async function handleWhatsAppStart(host: LobsterApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: LobsterApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: LobsterApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: LobsterApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: LobsterApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: LobsterApp): string {
...
function resolveGatewayHttpAuthHeader(host: LobsterApp): string | null {
...
function buildGatewayHttpHeaders(host: LobsterApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: LobsterApp) {
...
export function handleNostrProfileToggleAdvanced(host: LobsterApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: LobsterApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: LobsterApp) {
  const state = host.nostrProfileFormState;
